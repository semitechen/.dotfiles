    ...
    config = function()
        ...
        statusline.section_filename = function()
            ...
            if vim.bo.modified then
                return filename .. '  %#StlDot#●%#MiniStatuslineFilename#'
            end
        ...
        statusline.section_diagnostics = function(args)
            ...
            local res = {}
            if errors > 0 then
                table.insert(res, '%#StlError# ' .. errors .. '%#MiniStatuslineDevinfo#')
            end
            if warns > 0 then
                table.insert(res, '%#StlWarn# ' .. warns .. '%#MiniStatuslineDevinfo#')
            end
            if hints > 0 then
                table.insert(res, '%#StlHint#󰌵 ' .. hints .. '%#MiniStatuslineDevinfo#')
            end
        ...
        statusline.section_diff = function(args)
            ...
            local res = {}
            if (summary.added or 0) > 0 then
                table.insert(res, '%#StlAdd# ' .. summary.added .. '%#MiniStatuslineDevinfo#')
            end
            if (summary.changed or 0) > 0 then
                table.insert(res, '%#StlMod# ' .. summary.changed .. '%#MiniStatuslineDevinfo#')
            end
            if (summary.removed or 0) > 0 then
                table.insert(res, '%#StlDel# ' .. summary.removed .. '%#MiniStatuslineDevinfo#')
            end
